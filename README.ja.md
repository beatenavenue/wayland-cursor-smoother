...
| `min_facing` | 0 | facing ratio がこのパーセントに満たない縁では、リダイレクトしません。0 ならすべてのデッドバンドが対象です。[次の節](#壁が想定内と感じられる条件についての仮説)を参照してください |
| `undo_window` | off | **ワープ**のあと、この秒数のあいだは逆向きに `threshold` だけ動かすと、ポインタが元いた場所へ正確に戻ります。有効にする前に[戻るとき](#戻るとき)の節を読んでください。`style = glide` では何もしません |

### 壁が想定内と感じられる条件についての仮説

Windows 11 の「ディスプレイ間でカーソルを簡単に移動させる」のような機能は、
専用のツールを入れてでも欲しいと考える人がいる一方で、同じ機能を不満に感じる
人もいるようです。これは単なる好みの違いというより、ディスプレイの配置によって
人の認知がどうなっているか、という問題ではないでしょうか。この節はその仮説と、
それをもとにした設定 `min_facing` の説明です。

![laptop and display](img/desk-setup.svg)

よくある例は、ラップトップに外付けモニタをつないだ場合です。2つの画面は高さが
違うため、ポインタが行き来できる部分（図の緑の枠）はときに細く、行き来は
面倒です。それでもこの配置で、ポインタに壁（その先に何もない縁）を越えて
ほしいと望まれることは少ないようです。ポインタがどこから次の画面へ渡るかは
明らかなので、操作に迷うことがあまりないからです。

ポインタで向かう次の画面がどこにあるのか、という頭の中の認知と、実際の操作が
ずれた状態が「引っかかり」として体験されます。こうしたツールが本当に解決
したいのは、このずれではないでしょうか。逆に、あって当たり前の壁を簡単に
越えてしまうと、それはそれで認知のずれを生み、不快な体験になります。人によって
意見が割れるのは、このためではないかと考えています。

では、どんな条件なら人は「ここに壁があって当たり前」と感じやすいのでしょうか。
本ツールではその手がかりとして、facing ratio という値を使います。

- **向かう先の画面**は、ポインタを送り込もうとしている隣の画面です。
- 向かう先の画面の、今いる画面に面した辺のうち、2つの画面が実際に接している
  部分を**開口部**と呼びます。ポインタはここを通って画面を行き来します。
- **facing ratio** は、その辺の長さに対して開口部が占める割合です。

facing ratio が低いとき、壁は体感上、想定内のものです。facing ratio が高く
なるにつれて、壁は想定外の引っかかりに感じられるのではないか、というのが
この仮説です。

![facing ratio](img/facing_ratio_20_vs_80.svg)

図の2つの例では、どちらも向かう先の画面（target）の辺の長さは150です。濃い
緑の線が開口部、灰色の線はその辺のうち何にも接していない部分です。紫の点は、
今いる画面（current）の、その先に何もない縁に押し当てられているポインタです。

- 左は、150のうち30だけが接しているので、facing ratio は20%です。向かう先の
  画面はほとんど横にずれていて、壁があるのは見た目どおりです。
- 右は、150のうち120が接しているので、facing ratio は80%です。向かう先の画面は
  ほぼ正面にあるのに、ポインタは壁で止まります。

3枚以上のモニタを使っていると、1つの環境に想定内の壁と想定外の壁が混ざって
いることもあるでしょう。本ツールでは、`min_facing` という設定でこれを調整
できます。

`min_facing` には、facing ratio の**下限**をパーセントで指定します。
facing ratio がこの値以上の縁ではリダイレクトし、下回る縁では何もしません。
何もしない縁では、本ツールがないときと同じように、ポインタは壁で止まります。
たとえば50%にすると、上の図の左（20%）はリダイレクトせず、右（80%）は
リダイレクトします。既定値は0で、このときはすべてのデッドバンドがリダイレクトの
対象です。

facing ratio を手で計算する必要はありません。`--check` は、すべてのデッドバンドの
facing ratio を表示し、`min_facing` によってリダイレクトしない区間も一覧にします。
設定ファイルに書く前に、そこで値を試してください。
...
リダイレクトが始まるのは、裏に何もない縁です。けれども着地するのは2つの
ディスプレイが接している場所なので、帰り道に壁はありません。ポインタを戻せば、
2つのディスプレイが接しているほかの縁と同じように、そのまま越えて戻ります。
押し込む必要はありません。

戻る先は、デッドバンドの端です。元いた場所ぴったりではありません。2つの間の
距離は、リダイレクトが縁に沿って滑った距離です。これは `glide` でも `warp` でも
同じです。

ワープの場合は、`undo_window` でこの差を埋められます。ワープのあとその秒数の
あいだ、逆向きに `threshold` カウントだけ動かすと、ポインタは元いた場所へ正確に
戻ります。ボタンを押すとこれは取り消されます。クリックしたということは、そこに
いるつもりだったということだからです。

**既定で無効なのは、ポインタがどこにあるかを確かめないからです。** 逆向きの
動きは、画面のどこで起きても数えられます。そのため、もう一方のディスプレイの
中ほどで少し修正しただけでも、そこで作業するつもりだったのに、ポインタが縁の
向こうへ戻されることがあります。
...
ワープのあと、この秒数のあいだは逆向きに `threshold` だけ動かすと、ポインタが
元いた場所へ正確に戻ります。

戻るためにこれは必要ありません。リダイレクトは2つのディスプレイが接している
場所に着地するので、帰り道に壁はありません。ポインタを戻せばそのまま越えます。
ただし出てくるのはデッドバンドの端で、元いた場所ぴったりではありません。この
設定はその差を埋めます。

既定で無効なのは、ポインタがどこにあるかを確かめないからです。逆向きの動きは、
画面のどこで起きても数えられます。そのため、縁から離れた場所で少し修正した
だけでも、ポインタが縁の向こうへ戻されることがあります。試すなら3くらいが
目安です。ボタンを押すとこれは取り消されます。クリックしたということは、そこに
いるつもりだったということだからです。
...
facing ratio がこの値（0から100）以上のときだけ、リダイレクトします。
facing ratio とは、隣のディスプレイの縁のうち、今いるディスプレイに接している
部分の割合を、パーセントで表したものです。0 なら常にリダイレクトします。
止まると思っていたところでポインタが別のディスプレイへ飛んでしまうなら、
上げてください。
