...
| `undo_window` | off | **ワープ**のあと、この秒数のあいだは逆向きに `threshold` だけ動かすと、ポインタが元いた場所へ正確に戻ります。有効にする前に次の節を読んでください。`style = glide` では何もしません |

### 壁が想定内と感じられる条件についての仮説
Windows11の「ディスプレイ間でカーソルを簡単に移動させる」のような機能は、わざわざ専用のツールを導入してでも解決したい問題であると同時にユーザによっては不満に感じる機能でもあるらしい。それは単に好みの問題という以上にセットアップ状況によって人間の認知のほうがどうなっているのかという問題ではないだろうか。

![laptop and display](img/desk-setup.svg)

よくある例としてラップトップに外付けモニタを接続する場合。ときにその高さの差から操作エリアの接続部は細く、カーソルの行き来は面倒であるにも関わらずこのケースでカーソルの壁越えが望まれることは少ないようだ。どこからカーソルが次の画面に渡るかは自明であるためマウス操作で悩むことはあまりないのだ。

カーソルを操作して向かうべき次の画面がどこにあるのか、人間の認知と操作がずれた状態が引っかかりとして体験される。これがこうしたツールが解決したい本当のニーズではないだろうか。あって当たり前の壁を容易に越えてしまうと、それはそれで認知のずれを引き起こし不快な体験となる。それがユーザ毎に意見が割れる原因ではないか。

どんな条件であれば人間は「壁があって当たり前」と感じやすくなるのか。本ツールでは向かう先の画面と繋がっている開口部が占める割合を `facing ratio` と名付けた。 facing ratio が低い場合は体感上壁は想定内の存在であるが facing ratio が高くなるにつれて壁が想定外の引っかかりに感じられるのではないか。

![facing ratio](img/facing_ratio_20_vs_80.svg)

3画面など多くのモニタを使う人にとってはひとつの環境に想定内の引っかかりと想定外の引っかかりが混在していることもあるだろう。本ツールでは `min_facing` という設定値でこれを調整可能とする。
向かう先の画面から今いる画面に繋がる開口部の割合の**下限**のパーセントを設定する。たとえば50%と設定した場合は上図のfacing ratio 20%側は処理対象外となりfacing ratio 80%側は本ツールの処理対象となる。
...
facing ratioがこの値以上であれば処理対象とする。初期値0。

facing ratio とは 向かう先の画面の辺のうち、今いる画面と繋がっている開口部の割合。パーセント。
