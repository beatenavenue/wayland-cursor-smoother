...
*Ease cursor movement between displays* で行っているのと同じことです。
...
インタフェースとして提示し、属性を共有します。つまりそのルールは、守ろうとしたはずのキーストロークを
明け渡します。udev が各ノードをすでにどう分類したかでマッチすれば、それを避けられ、
しかもマウスを買い換えても生き残ります。
...
- **KWin のスクリプティングAPIは安定性の約束ではありません。** Plasma の
  ...
  それを教えます。
- **異なるDPIが混在する配置は未検証です。** 開発機のディスプレイはすべて
  `Scale: 1` を報告するので、そこでは論理ピクセルと物理ピクセルが一致しています。
  コードは一貫して KWin の論理座標で動いており、それで正しいはずですが、
  スケールされたディスプレイに対して動かしたことはありません。
- **1台のマシン、1つの構成。** 日常的に使われているのは、ちょうど1つの机の上です。
