...
facing ratio がこの値以上の縁でだけ、リダイレクトします。値はパーセントで、
既定値は0です。0 ならすべてのデッドバンドがリダイレクトの対象になります。
下回る縁では何もせず、ポインタは本ツールがないときと同じように壁で止まります。

facing ratio とは、向かう先の画面の、今いる画面に面した辺のうち、2つの画面が
実際に接している部分（開口部）が占める割合です。詳しくは
[壁が想定内と感じられる条件についての仮説](#壁が想定内と感じられる条件についての仮説)
を参照してください。
